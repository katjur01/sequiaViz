...
```
project_root/                           ← select this as root
├── somatic_mutation_loads.tsv          ← shared in root, "somatic" must be in the name (in Options A and B the folder already provides it)
├── patient_001/
│   ├── somatic_variants.tsv
│   ├── germline_variants.tsv
│   ├── fusions.tsv
...
::: {.alert .alert-info}
GnomAD column is the population allele frequency that SeqUIaSCOPE uses to filter variants by how
common they are. The preferred reference is `gnomAD_NFE`. If it is not present, the application
falls back to the first available column from a priority list and uses it as the main frequency
reference for filtering. Column matching is case insensitive. From gnomAD it accepts `gnomAD_NFE`,
`gnomAD_AFR`, `gnomAD_AMR`, `gnomAD_EAS`, `gnomAD_SAS`, `gnomAD_FIN`, `gnomAD_ASJ`, `gnomAD_MID`,
and the overall `gnomAD_AF`. From 1000 Genomes it accepts `1000g_eur_af`, `1000g_amr_af`,
`1000g_afr_af`, `1000g_eas_af`, `1000g_sas_af`, and the overall `1000g_af`. Your file needs at
least one of these, and the filter label in the module adjusts to whichever column was found.
:::
...
| `arriba.reading_frame` | Reading frame of the fusion predicted by Arriba (added automatically if absent) |
